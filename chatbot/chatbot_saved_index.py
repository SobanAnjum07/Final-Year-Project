...
def create_or_update_faiss_index(pdf_dir, chunk_size=1000, overlap=200):
    """Create or update the FAISS index with new PDFs."""
    index, metadata = load_faiss_index()
    
    # Get all PDFs in directory
    pdf_files = {f: os.path.join(pdf_dir, f) for f in os.listdir(pdf_dir) if f.endswith(".pdf")}
    existing_pdfs = set(metadata.keys())
    new_pdfs = set(pdf_files.keys()) - existing_pdfs  # Detect new PDFs

    if not new_pdfs and index:
        print("✅ No new PDFs found. Using existing FAISS index.")
        return index
    ...
    for pdf_name in new_pdfs:
        ...
        try:
            ...
            metadata[pdf_name] = len(chunks)  # Save chunk count per PDF
